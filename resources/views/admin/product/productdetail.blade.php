        ...
        <div class="flex-1 flex flex-col overflow-auto">
            ...
            <main class="p-6 flex-1 overflow-y-auto">
                <div class="container">
                    ...
                    <div class="mb-4">
                        <a href="{{ route('product.details.create', ['product_id' => $product_id]) }}" class="btn btn-primary">Thêm Chi Tiết Sản Phẩm</a>
                    </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr class="border">
                                ...
                                <th class="border">Discount Rate</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border">
                                ...
                                <td  class="border">
                                    <div class="carousel-container relative w-[500px] h-[400px] max-w-full overflow-hidden" id="carousel-{{ $productDetailId }}">
                                        <div class="carousel-wrapper flex transition-transform duration-500 ease-in-out">
                                            @foreach($productDetails as $image)
                                            <div class="flex-shrink-0 w-[300px]"> <!-- Đảm bảo có width cho mỗi slide -->
                                                <img src="{{ asset('storage/' . $image->image_url) }}" class="object-cover rounded-lg shadow carouselimg" alt="Product Image">
                                            </div>
                                            @endforeach
                                        </div>

                                        <!-- Nút điều hướng -->
                                        <button class="absolute left-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white p-2 rounded-full prev-btn">❮</button>
                                        <button class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white p-2 rounded-full next-btn">❯</button>
                                    </div>
                                </td>
                                <td>
                                    <div class="dropdown dropdown-end">
                                        ...
                                        <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow rounded-box w-52 bg-gray-300">
                                            <li>
                                                <a href="{{ route('product.details.edit', $pds->product_detail_id) }}">✏️ Edit</a>
                                            </li>
                                            <li>
                                                <a href="">📋 Restock List</a>
                                            </li>
                                            <li>
                                                <form action="{{ route('product.details.update.status', $pds->product_detail_id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="status" value="discontinued">
                                                    <button type="submit" class="text-yellow-500 hover:text-red-700" onclick="return confirm('Bạn có chắc chắn muốn cập nhật trạng thái sản phẩm này thành Discontinued?');">🔄 Discontinued</button>
                                                </form>
                                            </li>
                                        </ul>
                                    </div>
